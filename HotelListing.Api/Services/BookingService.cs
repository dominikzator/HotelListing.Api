...
public class BookingService(HotelListingDbContext context, IUsersService usersService, IMapper mapper) : IBookingService
{
    public async Task<Result<IEnumerable<GetBookingDto>>> GetBookingsForHotelAsync(int hotelId)
    {
        ...
        var bookings = await context.Bookings.Where(b => b.HotelId == hotelId).OrderBy(b => b.CheckIn).ProjectTo<GetBookingDto>(mapper.ConfigurationProvider).ToListAsync();
    ...
    public async Task<Result<IEnumerable<GetBookingDto>>> GetUserBookingsForHotelAsync(int hotelId)
    {
        ...
        var bookings = await context.Bookings.Where(b => b.HotelId == hotelId && b.UserId == userId).OrderBy(b => b.CheckIn).ProjectTo<GetBookingDto>(mapper.ConfigurationProvider).ToListAsync();
    ...
    public async Task<Result<GetBookingDto>> CreateBookingAsync(CreateBookingDto dto)
    {
        ...
        var hotel = await context.Hotels.Where(h => h.Id == dto.HotelId).FirstOrDefaultAsync();
        ...
        var created = new GetBookingDto(booking.Id, 
            hotel.Id,
            hotel.Name,
            dto.CheckIn,
            dto.CheckOut,
    ...
    private async Task<bool> IsOverlap(int hotelId, string userId, DateOnly checkIn, DateOnly checkOut, int bookingId = -1)
    {
        ...
        if(bookingId != -1)
        {
            query = query.Where(q => q.Id != bookingId);
        }
    ...
    public async Task<Result> CancelBookingAsync(int hotelId, int bookingId)
    {
        ...
        if (booking.Status == BookingStatus.Cancelled)
        {
            return Result.Failure(new Error(ErrorCodes.Conflict, $"Cancelled bookings cannot be modified."));
        }
    ...
    public async Task<Result> AdminConfirmBookingAsync(int hotelId, int bookingId)
    {
        ...
        if (booking.Status == BookingStatus.Cancelled)
        {
            return Result.Failure(new Error(ErrorCodes.Conflict, $"Booking has already been cancelled."));
        }
